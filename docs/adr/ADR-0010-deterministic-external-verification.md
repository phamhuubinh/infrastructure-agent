...
Accepted; controller boundary amended after the Agent v2 cutover.
...
limits, and provenance non-auditable. The controller may select a validated
Internet action when fresh external evidence is required, but verification
authority remains in the harness.

---

# Decision

For the primary path, the bounded controller selects structured decisions and
validated typed actions. The harness validates exact source authority. Stable
general knowledge can be answered without collectors; live environment requests
use the deterministic investigation pipeline; validated current/external
requests and explicit public URLs use the deterministic external-verification
path.
...
controller/assessment model does not receive a direct web-tool API and cannot
choose a provider, bypass URL validation, control arbitrary retries, or expand
the verification budget. Explicit user URLs and typed source constraints are
hard inputs validated by code.
...
current claim. Controller/model failure never falls back to model memory as if it
...
- Natural-language currentness reasoning can use the controller while source
  enforcement, cost, and stop conditions remain repeatable.
...
- Currentness/source action selection depends on valid typed controller output;
  malformed, unavailable, or unsupported output fails closed rather than using
  the old lexical router.
