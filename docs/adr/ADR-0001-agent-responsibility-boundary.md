...
The Agent executes actions generated by the deterministic pipeline.
...
**Note (2026-07-26):** The execution model has evolved from a model-driven iterative loop to a deterministic single-pass pipeline (see ADR-0007). The Agent still executes deterministically without reasoning — the change is that execution is now a pre-planned pipeline rather than a model-guided iteration.
---
# Responsibilities
...
- ADR-0007 (`docs/adr/ADR-0007-deterministic-pipeline.md`) — supersedes the execution model in this ADR; documents the actual deterministic single-pass pipeline
...
**Note (2026-07-26):** The execution model is now a deterministic single-pass pipeline (see ADR-0007), not an iterative Action → Observation loop. The architecture remains model-agnostic — the assessment model can be swapped without changing the deterministic pipeline.