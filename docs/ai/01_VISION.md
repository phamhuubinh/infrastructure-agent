# 01 - Vision
## What this is today
An **Infrastructure Investigation Agent**: a local, single-user tool that investigates infrastructure (via SSH, Grafana, Zabbix) deterministically, collects evidence, and uses an LLM only to interpret that evidence and produce an assessment. It runs on one machine, has no accounts, no remote database, and no network exposure beyond outbound calls it makes to targets/Grafana/Zabbix/LLM APIs.
...
- The Agent's tools (SSH execution, Grafana, Zabbix, future Internet Tool) keep their current design contract (see `06_TOOL_AND_CAPABILITY_DESIGN.md`) — becoming a platform component does not change how a tool declares its capability.
- Internet access from the Agent, when it exists, is opt-in per request, never automatic (see `04_ROADMAP.md`, WP4).
...
None of this is implemented yet. Do not build against it until `08_PROJECT_STATE.md` says a work package is done.
