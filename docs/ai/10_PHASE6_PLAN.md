...
| 602 | `DeterministicAgent.run()` and `run_with_steps()` catch unknown/ambiguous targets separately and never use the chat fallback. Target/trace tests exercise both paths. | DR1-306/309 complete: unresolved targets now produce bounded deterministic clarification before collection. |
| 603 | `TargetResolver` validates exact/alias/pattern candidates before fuzzy matching and has focused unknown-target tests. | DR1-306 complete: pure alphabetic and Vietnamese-preposition targets use threshold + margin + unknown-target guards and cannot silently reach `localhost`. |
...
| 605 | `CapabilityPlanner` is invoked from both execution paths and filters existing `capability_references` when names overlap. | DR1-301/302/308 complete: routing has no LLM classifier fallback and both paths consume the canonical `RequestFrame` plus first-class statuses. |
| 606 | `concepts.yaml` contains standalone hostname, kernel, uptime, and load concepts; port is represented through network/firewall synonyms. | DR1-303 complete: deterministic typo/code-switch candidates and reviewed lexicon variants are present. Remaining concept breadth and bounded decomposition belong to DR1-405/802. |
| 607 | `capability_plans.yaml` contains plans for the Phase 6 concepts and `test_capability_planner.py` checks their local mappings. | Canonical planning input/status wiring is complete under DR1-302/308; parameter binding and bounded multi-request decomposition remain under DR1-403/405. |
...
| 610 | Both engine paths store extracted parameters on the investigation/state and pass the object into `ExecutionRuntime.execute()`. | DR1-302 unified the request frame; structured cross-turn context and capability binding remain under DR1-402/403. |
...
| 612 | `AnswerType` and its deterministic classifier exist with focused unit tests. | DR1-308 complete: fact/list/table/chart/assessment/comparison/forecast/action/explanation are canonical; temporal evidence guards remain under DR1-406/407. |
| 613 | Both engine paths classify and store `answer_type`. | DR1-308 complete: routing/evidence/answer strategy and LLM usage reason are authoritative first-class trace fields. |
...
| 615 | `ToolSelector` and `ToolCategory` exist; focused tests cover directive/concept selection. | DR1-301/305/308 now validate routing confidence/candidates before selection; the selector itself remains a category mapper and its unknown-input default is not routing authority. |
| 616 | Both engine paths call `ToolSelector.select()` and store `selected_tool`. | End-to-end route authority is deterministic after DR1-301/302/308; `selected_tool` remains mainly a merge label rather than direct `CapabilityRouter` authority. |
...
| 632 | `_build_tool_links()` resolves a range and appends available tool links after assessment. | DR1-308 supplies the strategy taxonomy; embed/image response and time-series sufficiency remain under DR1-407/707. |
