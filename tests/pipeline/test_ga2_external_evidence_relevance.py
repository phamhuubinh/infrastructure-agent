...
class TestLegacyTestsStillPass:
    """Verify that legacy 10 tests in test_external_verification.py remain present and green.

    This class documents the legacy test count to ensure no regression.
    """

    def test_legacy_test_count_in_external_verification_file(self) -> None:
        """Verify that test_external_verification.py still contains the legacy tests."""
        import os
        ...
        assert os.path.exists(test_file), "Legacy test file must exist"

        with open(test_file) as f:
            content = f.read()

        # Count test functions (not methods inside classes)
        import re

        # Legacy standalone test functions
        legacy_functions = [
            "test_search_select_fetch_normalizes_fresh_external_evidence",
            "test_explicit_url_fetches_directly_without_search",
            "test_fetch_success_with_empty_content_is_not_verified_evidence",
            "test_unavailable_search_never_turns_into_model_or_fetch_evidence",
            "test_failed_fetch_is_not_cached_as_valid_evidence",
            "test_page_fetch_budget_and_byte_limit_are_propagated",
            "test_explicit_private_url_still_hits_the_shared_ssrf_boundary",
        ]

        for func in legacy_functions:
            assert func in content, f"Legacy test {func} must be preserved"

        # Verify at least 7 standalone legacy tests exist
        standalone_tests = re.findall(r"^def (test_\w+)\(", content, re.MULTILINE)
        assert (
            len(standalone_tests) >= 7
        ), f"Expected at least 7 standalone legacy tests, found {len(standalone_tests)}"
