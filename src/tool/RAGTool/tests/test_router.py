    ...
    def test_log_sanitization_with_valid_path(self):
        ...
        with patch("src.tool.RAGTool.app.parsers.router.logger") as mock_logger:
            ...
            call_args = mock_logger.info.call_args[0][
                0
            ]  # Get the format string from the first call argument
            assert "Processed file with fallback parser" in call_args
            assert (
                "test_file.pdf" not in call_args
            )  # Should not contain actual filename
            assert (
                "[No file path provided]" not in call_args
            )  # Should not contain the empty path message
